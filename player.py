...
class Player(pygame.sprite.Sprite):
    def __init__(self, pos, surface, create_jump_particles, change_health):
        ...
        dust_run_particles_path = 'graphics/character/dust_particles/run'
    ...
    def import_character_assets(self):
        character_path = 'graphics/scheleton/'
    ...
    def animate(self):
        ...
        if self.facing_right:
            ...
            flipped_image = pygame.transform.flip(image, True, False)  # .flip(img, x, y)
    ...
    def get_input(self):
        ...
        if keys[pygame.K_SPACE] and self.on_ground:
            self.jump()
            self.create_jump_particles(self.rect.midbottom - pygame.Vector2(0, 15))

    def get_status(self):
        if self.direction.y < 0:
            self.status = 'jump'
        elif self.direction.y > self.gravity + 0.2:
            self.status = 'fall'
        else:
